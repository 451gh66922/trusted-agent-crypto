...
> **实现定位（对齐 06 版进度安排 v2 / 2026-09-08）**：本仓库以 **SoftSE 软件安全元件** 为主路径实现可信授权；原 R1 硬件选型（NXP SE）已取消，仅以 **SoftSE + 可插拔模拟（卸载后端 = 拔卡）** 形态呈现，硬件差距与移植点在说明书局限章交代。TPM / 真 SE 仅作 `CryptoBackend` 接口层的占位 stub，不作为主交付物。

**主语言：Python ≥ 3.11**（SoftSE / DPoP / 策略门 / 审计日志 / pytest 全栈统一；真 SE 侧可用 APDU/PKCS#11 适配，不必换语言）。
...
课题说明书、进度安排、参考文献放在本地桌面目录，**不提交本仓库**（见 `.gitignore`）。
...
- 正常链路：SoftSE 签 DPoP proof → Keycloak 登录 → 换 DPoP-bound token → 调受保护 API 200
- 攻击①泄露：软件基线可读私钥文件得手 / SoftSE 无导出一无所获
- 攻击②挪用：`tokens.json` 拷走重放 401（jkt 发送方约束）
- 攻击③滥用：无策略越权成功作反衬 / 有策略在密钥使用点 `POLICY_DENIED` 并落日志
- 插拔：卸载后端即失败、重挂即恢复（模拟拔卡）；销毁密钥则旧授权立即失效
